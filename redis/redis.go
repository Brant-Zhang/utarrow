...
func NewRedisManager(host string, poolsize int, timeout time.Duration) (p *RedisManager, err error) {
	p = &RedisManager{}
	if poolsize == 0 {
...
func (w *RedisManager) Rpush(key string, content interface{}) (err error) {
	action := func(conn redis.Conn) (interface{}, error) {
		return conn.Do("RPUSH", key, content)
	}
	_, err = w.Do(action)
	return
}
...
// Zremrangebyscore zset rem
func (w *RedisManager) Zremrangebyscore(key string, min, max int64) (del int, err error) {
	action := func(conn redis.Conn) (interface{}, error) {
...
//diffrent return data type with upstairs function
func (w *RedisManager) ZrevrangeByScore(key string, start, end int) (result [][2]string, err error) {
...
// ZrangeByScore 通过score值范围获取列表
func (w *RedisManager) ZrangeByScore(key string, min int, max int) (rankList []string, err error) {
...
// ZrangeByScoreWithScore 排名列表 with score
func (w *RedisManager) ZrangeByScoreWithScore(key string, min, max int) (sortedKeys []string, result map[string]float64, err error) {
