...
func AesEncryptPkcs7(origData, key, iv []byte) ([]byte, error) {
...
func AesDecryptPkcs7(crypted, key, iv []byte) ([]byte, error) {
	block, err := aes.NewCipher(key)
	if err != nil {
		return []byte(""), err
	}
...
func AesDecryptKeepPadding(crypted, key []byte, iv []byte) ([]byte, error) {
	block, err := aes.NewCipher(key)
	if err != nil {
		return []byte(""), err
	}
...
func AesEnPKCS7(origData, key []byte, iv []byte) (int, error) {
	block, err := aes.NewCipher(key)
	if err != nil {
		return 0, err
	}
	//blockSize := block.BlockSize()
	//fmt.Printf("-----------------before padding:%x\n", origData)
	origData = pkcs7Padding(origData)
	//fmt.Printf("----------------after padding:%x\n", origData)
	blockMode := cipher.NewCBCEncrypter(block, iv)
	//crypted := make([]byte, len(origData))
	blockMode.CryptBlocks(origData, origData)
	//fmt.Println(origData, len(origData))
	return len(origData), nil
}

func AesDePKCS7(crypted, key []byte, iv []byte) (int, error) {
	//fmt.Printf("###############input:%v\nlength:%d\n", crypted, len(crypted))
	block, err := aes.NewCipher(key)
	if err != nil {
		return 0, err
	...
	if len(crypted)%16 != 0 {
		return 0, errors.New("cipher len not 16s")
	}
	blockMode := cipher.NewCBCDecrypter(block, iv)
	...
	return len(crypted), nil
}

func pkcs7UnPadding(originData []byte) ([]byte, error) {
	//fmt.Printf("before unpadding:%x\n", originData)
	...
	//fmt.Printf("after unpadding====%x=\n", originData[:bytelen-unpadding])
